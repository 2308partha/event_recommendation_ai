...
export const ChatbotDrawer: React.FC = () => {
  ...
  const handleSendMessage = async (textToSend: string) => {
    ...
    try {
      ...
      const reply = await sendChatMessage(sessionId, textToSend);
      ...
      // API call failed but api.ts automatically handles offline fallback,
      // so this block is rarely hit unless something critical breaks.
      setMessages(prev => [...prev, {
        id: `error_${Date.now()}`,
        sender: 'bot',
        text: "Sorry, I ran into a connection glitch. Feel free to check if the FastAPI port 8000 is open!",
        timestamp: new Date()
      }]);
    } finally {
      setLoading(false);
